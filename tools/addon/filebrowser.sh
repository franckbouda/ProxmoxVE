...
DB_PATH="/usr/local/franckbouda/filebrowser.db"
DEFAULT_PORT=8080
...
if [[ "${install_prompt,,}" =~ ^(y|yes)$ ]]; then
    ...
    mkdir -p /usr/local/franckbouda
    chown root:root /usr/local/franckbouda
    chmod 755 /usr/local/franckbouda
    touch "$DB_PATH"
...
[Service]
User=root
WorkingDirectory=/usr/local/franckbouda
ExecStartPre=/bin/touch /usr/local/franckbouda/filebrowser.db
ExecStartPre=/usr/local/bin/filebrowser config set -a "0.0.0.0" -p ${PORT} -d /usr/local/franckbouda/filebrowser.db
ExecStart=/usr/local/bin/filebrowser -r / -d /usr/local/franckbouda/filebrowser.db -p ${PORT}
Restart=always
...
directory="/usr/local/franckbouda"
